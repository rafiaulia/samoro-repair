...
const ADM_SUPABASE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imh3a2hrZ2ltYXJlZnNvbWJnbXduIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU5OTc0ODEsImV4cCI6MjA5MTU3MzQ4MX0.icCN7QfuKdXuTUU5G2DNB_Xed4BeLRJI1TY_VZusNBU";
...
(function admInitSupabase() {
  ...
  window._admDb = window.supabase.createClient(
    ADM_SUPABASE_URL,
    ADM_SUPABASE_KEY,
  );
})();
...
async function admLogin(e) {
  ...
  const { data, error } = await window._admDb.auth.signInWithPassword({
    email,
    password,
  });
...
function admUpdateStats(orders) {
  ...
  admSetTeks(
    "adm-stat-masuk",
    orders.filter((o) => o.status === "masuk").length,
  );
  admSetTeks(
    "adm-stat-proses",
    orders.filter((o) => o.status === "proses perbaikan").length,
  );
  admSetTeks(
    "adm-stat-siap",
    orders.filter((o) => o.status === "siap diambil").length,
  );
  admSetTeks(
    "adm-stat-selesai",
...
document
  .getElementById("adm-stat-total")
  ?.closest(".adm-stat-card")
  .addEventListener("click", () => (location.href = "pesanan.html"));
document
  .getElementById("adm-stat-masuk")
  ?.closest(".adm-stat-card")
  .addEventListener(
    "click",
    () => (location.href = "pesanan.html?filter=masuk"),
  );
document
  .getElementById("adm-stat-proses")
  ?.closest(".adm-stat-card")
  .addEventListener(
    "click",
    () => (location.href = "pesanan.html?filter=proses+perbaikan"),
  );
document
  .getElementById("adm-stat-siap")
  ?.closest(".adm-stat-card")
  .addEventListener(
    "click",
    () => (location.href = "pesanan.html?filter=siap+diambil"),
  );
document
  .getElementById("adm-stat-selesai")
  ?.closest(".adm-stat-card")
  .addEventListener(
    "click",
    () => (location.href = "pesanan.html?filter=selesai"),
  );
/**
...
function admFilterTabel() {
  ...
  const filtered = _admOrders.filter(
    (o) =>
...
async function admSubmitOrder(e) {
  ...
  // Validasi
  if (!resi) {
    admTampilError("adm-form-error", "Nomor resi wajib di-generate!");
    return;
  }
  if (!nama) {
    admTampilError("adm-form-error", "Nama customer wajib diisi!");
    return;
  }
  if (!device) {
    admTampilError("adm-form-error", "Nama perangkat wajib diisi!");
    return;
  }
  if (!layanan) {
    admTampilError("adm-form-error", "Jenis layanan wajib diisi!");
    return;
  }
  if (!keluhan) {
    admTampilError("adm-form-error", "Keluhan wajib diisi!");
    return;
  }
  ...
  if (id) {
    ...
    ({ error } = await window._admDb.from("orders").insert([
      {
        ...payload,
        waktu_masuk: new Date().toISOString(), // ← tambah ini
      },
    ]));
  }
  ...
  admToast(
    id ? "✓ Order berhasil diperbarui!" : "✓ Order baru berhasil ditambahkan!",
  );
  admResetForm();
  admMuatOrder();
}
...
async function admQuickStatus(id, status) {
  ...
  if (order) {
    order.status = status;
    if (kolom) order[kolom] = payload[kolom];
...
function admEditOrder(id) {
  ...
  if (!printBtn) {
    ...
    printBtn.onclick = () =>
      admCetakNota(parseInt(document.getElementById("adm-edit-id").value));
    document
      .getElementById("adm-submit-btn")
...
async function admHapusOrder(id, resi) {
  ...
  const { error } = await window._admDb.from("orders").delete().eq("id", id);
...
const ADM_NOTA_STATUS_COLORS = {
  masuk:
    "background:var(--adm-blue-50);color:var(--adm-blue-700);border-color:var(--adm-blue-200)",
  "proses perbaikan":
    "background:var(--adm-yellow-50);color:var(--adm-yellow-600);border-color:var(--adm-yellow-100)",
  "siap diambil":
    "background:var(--adm-purple-50);color:var(--adm-purple-600);border-color:var(--adm-purple-100)",
  selesai:
    "background:var(--adm-green-50);color:var(--adm-green-600);border-color:var(--adm-green-100)",
};
...
function admCetakNota(id) {
  ...
  document.getElementById("adm-nota-tgl").textContent = admFormatTanggal(
    o.created_at,
  );
...
document.addEventListener("DOMContentLoaded", async function admInit() {
  ...
  const {
    data: { session },
  } = await window._admDb.auth.getSession();
  if (session) {
